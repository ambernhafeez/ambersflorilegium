---
layout: splash
title: "Science"
permalink: /science/
---

<div class="florilegium-page">

  <header class="florilegium-page-header">

    <h1>Science</h1>

    <p class="page-subtitle">
      Plant pathology · Crop genetics · Research
    </p>

    <div class="florilegium-rule"></div>

  </header>


  <!-- INTRODUCTION -->

  <div class="florilegium-page-intro">

    <p>
      I am a scientist with a background in plant pathology, crop genetics
      and disease resistance. My research has focused particularly on the
      interactions between plants and the pathogens that affect them.
    </p>

  </div>


  <!-- RESEARCH -->

  <section class="science-section">

    <h2>Research</h2>

    <p>
      My research has included:
    </p>

    <ul class="science-research-list">

      <li>Plant pathology and plant–pathogen interactions</li>

      <li>Genetics of crop disease resistance</li>

      <li>Genomics and bioinformatics</li>

      <li>Cereal pathology (wheat and its wild relatives and landraces)</li>

      <li>Phytopathology and gene mapping projects in potatoes and legumes</li>

      <li>Working with scientists adn breeding industry stakeholders to develop a prooposal for sustainable disease resistance gene deployment</li>

    </ul>

  </section>

 <!-- ACADEMIC PROFILES -->

  <section class="science-section science-profiles">

    <h2>Academic profiles</h2>

    <p>
      Find my publications and research record:
    </p>

    <div class="science-profile-links">

      <a href="https://orcid.org/my-orcid?orcid=0000-0003-4692-5001" target="_blank" rel="noopener">
        ORCID →
      </a>

      <a href="https://scholar.google.com/citations?user=MXFrsCoAAAAJ&hl=en" target="_blank" rel="noopener">
        Google Scholar →
      </a>

    </div>

  </section>

  <!-- PUBLICATIONS -->

  <section class="science-section">

    <h2>Selected publications</h2>

    <div class="science-publications">

      <article class="science-publication">

        <h3>Septoria tritici blotch resistance gene Stb15 encodes a lectin receptor-like kinase</h3>

        <p>
          <strong>Amber N. Hafeez</strong>, et al.
          <em>Nature Plants</em> (2025).
        </p>

        <a href="https://www.nature.com/articles/s41477-025-01920-2">
          Read publication (open access) →
        </a>

      </article>


      <article class="science-publication">

        <h3>Creation and judicious application of a wheat resistance gene atlas</h3>

        <p>
          <strong>Amber N. Hafeez</strong>, et al.
          <em>Molecular Plant</em> (2021).
        </p>

        <a href="https://www.cell.com/molecular-plant/fulltext/S1674-2052(21)00175-1">
          Read publication (open access) →
        </a>

      </article>


      <article class="science-publication">

        <h3>Harnessing landrace diversity empowers wheat breeding</h3>

        <p>
          Shifeng Cheng, et al.
          <em>Nature</em> (2024).
        </p>

        <a href="https://www.nature.com/articles/s41586-024-07682-9">
          Read publication (open access) →
        </a>

      </article>

    </div>

  </section>


  <!-- ACADEMIC CAREER -->

  <section class="science-section">

    <h2>Academic career</h2>

    <div class="science-timeline">

      <div class="science-timeline-item">

        <p class="science-date">
          20XX–20XX
        </p>

        <h3>Position or degree</h3>

        <p class="science-institution">
          University / Institution
        </p>

        <p>
          
        </p>

      </div>


      <div class="science-timeline-item">

        <p class="science-date">
          20XX–20XX
        </p>

        <h3>Position or degree</h3>

        <p class="science-institution">
          University / Institution
        </p>

        <p>
          
        </p>

      </div>


      <div class="science-timeline-item">

        <p class="science-date">
          20XX–20XX
        </p>

        <h3>Position or degree</h3>

        <p class="science-institution">
          University / Institution
        </p>

        <p>
          
        </p>

      </div>

    </div>

  </section>


</div>